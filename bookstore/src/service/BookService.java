...
public class BookService {
	...
	public List<Book> queryBooks (@QueryParam("title") final String title) {
		...
		final List<Book> searchedBooks = entityManager.createQuery("SELECT b FROM Book b WHERE (:title IS NULL OR b.title LIKE :title)", Book.class).setParameter("title", title == null ? null : "%" + title + "%").getResultList();
